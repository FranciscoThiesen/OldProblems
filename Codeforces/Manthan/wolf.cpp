...
bool ok(int idx){
	int found = 0;
	graph<int> G(5*n + 1); // dummy value sz <= 50
	// source is idx = 0
	cout << "criei o grafo " << endl;
	FU(i, 1, n+1){
		if(vec[i].fi == idx || vec[i].se == idx) found++;
		else{
			G.arc(0,i,1);
			G.arc(i, n+vec[i].fi, 1);
			G.arc(i, n+vec[i].se, 1);
			printf("criei aresta a(%d, %d) cap = %d\n", 0, i, 1);
			printf("criei aresta b(%d, %d) cap = %d\n", i, n+vec[i].fi, 1);
			printf("criei aresta c(%d, %d) cap = %d\n", i, n+vec[i].se, 1);
		}
	}

	FU(i, 1, n+1) G.arc(n+i, 5*n, found-1);
	ll mf = G.maxflow(0, 5*n);
	cout << "maxflow que encontrei foi " << mf << endl;
	cout << "found = " << found << " n = " << n << " mf = " << mf << endl;
	if(mf == n - found){
		return true; // perdeu
	}
	return false;
