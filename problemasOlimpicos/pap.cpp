...
int main()
{
	while(true)
	{
		...
		if(l + col + instructions == 0)
			return 0;
		memset(mat, '\0', sizeof(char) * MAXN * MAXN);
		...
		F(r,0,instructions)
		{
			...
			else
			{
				if(currentOrientation == 'S')
				{
					if(mat[xCurrent][yCurrent+1] != '\0' && mat[xCurrent][yCurrent+1] != '#')
					{
						yCurrent++;
				...
				{
					if(mat[xCurrent+1][yCurrent] != '\0' && mat[xCurrent+1][yCurrent] != '#')
					{
						xCurrent++;
						if(mat[xCurrent][yCurrent] == '*')
						{
