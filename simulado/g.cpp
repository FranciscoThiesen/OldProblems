...
int main(){
	...
	while(t--){
		...
		polygon v = convex_hull(pts);
		if((int)v.size() == n){
			bool ok = true;
			v.pb(v[0]);
			double ang = angle(v[0], v[1], v[2]);
			double side = abs(v[0] - v[1]);
			for(int i = 2; i < n+1; ++i){
				double angT = angle(v[i-2], v[i-1], v[i]);
				//cout << "angulo intermediario = " << angT << endl;
				if(cmp(ang, angT) != 0) ok = false;
			}
			for(int i = 1; i < n+1; ++i){
				double sideT = abs(v[i] - v[i-1]);
				//cout << "lado intermediario = " << sideT << endl;
				if(cmp(sideT, side) != 0) ok = false;
			}
			if(ok) cout << "YES" << endl;
			else cout << "NO" << endl;
		}else{
