...
#define _for(i, a, b) for (int(a) i = (a); i != (b); ++i)
#define foreach(x...) _for(x)
#define forall(i, v) foreach(i, all(v))
// If using C++11, change __typeof__ to decltype
#define FU(i, a, b) for(int(a) i = (a); i < (b); ++i)
...
int visited[maxn] = {0};
int color[maxn] = {0};
vector<vector<int> > adjList(1e6 + 10);
int mxCol = 0;
...
int dfs_color(int root, int col){
	color[root] = col;
	visited[root] = 1;
	mxCol = max(col, mxCol);
...
int main(){
	...
	while(t--){
		...
		fu(i, n){
			...
			segs.pb(mp({x1,y1},{x2,y2}));
		}
		map<pair<int, int>, int > indiceProPonto;
		int globIdx = 0;
		fu(retaSt, n){

			vector<point> vizinhos;
			FU(reta2, retaSt+1, n){
				bool ok = seg_intersect(segs[retaSt[i]].fi, segs[retaSt[i]].se, segs[reta2].fi, segs[reta2].se); 
				if(ok){
					point p = line_intersect(segs[retaSt[i]].fi, segs[retaSt[i]].se, segs[reta2].fi, segs[reta2].se);
					vizinhos.pb(p);
				}	
			}
			sort(all(vizinhos));
			...
			for(const point& p : vizinhos){
				if(indiceProPonto.find((mp(p.x, p.y))) == indiceProPonto.end()){
					indiceProPonto[mp(p.x, p.y)] = ++globIdx;
					idx.pb(globIdx-1);
				}
				else idx.pb(indiceProPonto[mp(p.x, p.y)]);
			}
			int tam = vizinhos.size();
			FU(i, 1, tam){
				adjList[idx[i]].pb(idx[i-1]);
				adjList[idx[i-1]].pb(idx[i]);
			}
		}
		fu(i, globIdx){
			if(visited[i] == 0){
